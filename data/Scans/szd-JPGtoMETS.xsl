...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <book xmlns="http://gams.uni-graz.at/viewer" xmlns:xlink="http://www.w3.org/1999/xlink">
                ...
                <date>
                	<xsl:choose>
                		...
                		<xsl:when test="matches($date, '\d{4}-\d{2}')">
                			<xsl:value-of select="concat(substring-after($date, '-'), '.', substring-before($date, '-'))"/>
                		</xsl:when>
                		<xsl:otherwise>
        	...
        	<!--  -->
            <structure>
            	<xsl:variable name="FolderName" select="normalize-space(substring-before(substring-after(base-uri(), 'Scans/'), '.xml'))"/>
            					...
            					<xsl:choose>
            						<xsl:when test="chapter">
            							<xsl:for-each select="chapter">
            								<div type="{title}">
	            								<xsl:variable name="number_with_leading_zero" select="substring(string(1000 + from), 2)"/>
            						...
            						<xsl:otherwise>
            							<xsl:for-each select="$page_begin to $page_end">
            								<xsl:variable name="number_with_leading_zero" select="substring(string(1000 + .), 2)"/>
            								<page xlink:href="{concat($filename, $number_with_leading_zero, '.jpg')}"/>
            							</xsl:for-each>
            						</xsl:otherwise>
            					</xsl:choose>
