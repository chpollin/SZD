	...
	<xsl:template name="getThema">
	    ...
	    <div class="row mt-5">
	        <div class="col-12">
	            ...
	            <a type="button" data-toggle="collapse" data-target="{concat('#viewdetails', generate-id(//t:body/t:div[@type = 'Einleitung'][@xml:lang = $local]/t:p[2]))}" style="color:#C2A360;">&#9660;</a>
	        </div>
	    </div>
	    
		<xsl:for-each select="//t:body/t:div[@type = 'Thema'][@xml:lang = $local]">
                          ...
                          <xsl:for-each select="tokenize($SZDID, '\s')">
                              ...
                              <xsl:choose>
                                  <xsl:when test="contains(., 'SZDMSK')">
                                      <a href="{concat('/o:szd.werke/sdef:TEI/get?locale=', $locale, '#', $helpSZDID)}">
                                  ...
                                  <xsl:when test="contains(., 'SZDLEB')">
                                      <a href="{concat('/o:szd.lebensdokumente/sdef:TEI/get?locale=', $locale, '#', $helpSZDID)}">
                                          <xsl:call-template name="printEnDe">
                                              <xsl:with-param name="locale" select="$locale"/>
