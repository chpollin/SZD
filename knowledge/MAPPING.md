...
One-to-one mapping from each column of the correspondence catalogue CSV (external catalogue data) to the TEI element, attribute or value inside the konvolut files. It documents the schema contract between catalogue metadata and TEI encoding. The CSV catalogue files live outside this repository.
...
* Empty CSV cells are considered “not asserted”, and their TEI counterpart
...
3. Report  
   One line per row lists the mismatches. OK rows may be silent or counted.
